    ...
    7: [function (require, module, exports) {
        ;(function (window, document, undefined) {
            ...
            */
            ...
            function toggleResultsContainer(parameter) {
                options.resultsContainer.style.display = parameter
            }
            ...
            function registerInput() {
                options.searchInput.addEventListener('keyup', function (e) {
                    ...
                    if (isWhitelistedKey(key) && isValidQuery(query)) {
                        ...
                        toggleResultsContainer('block')
                        render(repository.search(query))

                    }
                    // 按下下方向键
                    if (key == 40) {
                        let allLi = options.resultsContainer.querySelectorAll('li')
                        let liNum = allLi.length
                    ...
                    if (key == 38) {
                        ...
                        for (let i = 0; i < liNum; i++) {
                            if (allLi[i].classList.contains('active')) {
                                allLi[i].classList.remove('active')
                                if (i == 0) {
                                    allLi[liNum - 1].classList.add('active')
                                } else {
                                    allLi[i - 1].classList.add('active')
                                }
                                break
                    ...
                    if (key == 13) {
                        ...
                        for (let i = 0; i < liNum; i++) {
                            if (allLi[i].classList.contains('active')) {
                                window.location.href = options.resultsContainer.querySelectorAll('a')[i].href
                                break
                            }
                        }
